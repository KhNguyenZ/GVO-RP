...
forward OnAdminDoorCreate(playerid, name[], Float:dx, Float:dy, Float:dz, pickup);
public OnAdminDoorCreate(playerid, name[], Float:dx, Float:dy, Float:dz, pickup)
{
    new iddoor = cache_insert_id();
    strcat(DoorInfo[iddoor][door_name], name, 40);
    DoorInfo[iddoor][door_id] = iddoor;
    DoorInfo[iddoor][door_ExPos][0] = dx;
    DoorInfo[iddoor][door_ExPos][1] = dy;
    DoorInfo[iddoor][door_ExPos][2] = dz;
    DoorInfo[iddoor][door_pickup] = pickup;
...
hook OnPlayerUpdate(playerid)
{
	...
	{
		ChangeGuideContent(playerid, "H", "Ra/Vao door");
	}
	return 1;
}